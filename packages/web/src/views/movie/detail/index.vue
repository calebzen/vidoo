<script setup lang="ts">
  ...
  const vidSrc = computed(() => `https://vidsrc.icu/embed/movie/${movieId.value}`);
  ...
  onMounted(async () => {
    details.value = await movie.details(movieId.value);
    document.title = details.value.title;
    credits.value = await movie.credits(movieId.value);
    const response = await movie.externalIds(movieId.value);
    externalIdList.value = Object.keys(response).filter(id => response[id]);
    similarMovieList.value = (await movie.similar(movieId.value)).results.slice(0, 5);
  });
        ...
        <div class="flex flex-col z-[1] p-8">
          <section class="base-info space-y-2">
            ...
            <div class="text-content space-x-2">
              <span>{{ minutesToMinutesHours(details.runtime) }}</span>
              <span class="relative before-dot">
                <span class="ml-3">{{ details.release_date }}</span>
              </span>
          ...
          <section class="mt-4 space-x-4">
            <RouterLink :to="`/watch/movie/${movieId}`" target="_blank">
              <button v-if="details.status === 'Released'" type="button"
    ...
    <div class="sub-container mt-4 flex gap-4">
      ...
      <div class="w-80 max-w-80">
        <section id="section-movie-stat">
          ...
          <div class="stats shadow-none stats-vertical max-w-80 min-w-80">
            <div class="stat">
              ...
              <div class="stat-value">{{ dateFmt.format(new Date(details.release_date)) }}</div>
            ...
            <div class="stat">
              <div class="stat-figure text-base-content size-8">
                <DollarSign class="size-8" />
              </div>
              <div class="stat-title">
                <span>Revenue</span>
                <span class="text-meta mx-2">/</span>
                <span class="text-meta">budget</span>
              </div>
              <div class="stat-value">
                <span> {{ numFmt.format(details.revenue) }}</span>
                <span class="text-meta mx-2">/</span>
                <span class="text-description">{{ numFmt.format(details.budget) }}</span>
              </div>
              <div class="stat-desc">Global revenue</div>
            </div>
          </div>
        </section>
        <section id="section-link">
          ...
          <div class="card">
            <ul class="menu">
              <li v-for="item in externalIdList" :key="item">
